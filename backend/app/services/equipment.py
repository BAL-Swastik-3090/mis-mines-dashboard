...
MTTR / MTBF
───────────
  MTTR (hrs) = SUM(BREAKDOWN_DURAION) ÷ COUNT(breakdown events)
  MTBF (hrs) = (period_hrs − SUM(BREAKDOWN_DURAION)) ÷ COUNT(breakdown events)
  Fleet MTTR = total_bd_hrs_all_machines ÷ total_bd_count_all_machines
  Fleet MTBF = (machines_with_bd × period_hrs − total_bd_hrs) ÷ total_bd_count
"""
...
def _get_bd_hours(
    ...
    """SAP breakdown hours + count for a given list of machine names.
    Returns {machine_name: {"hours": float, "count": int}}
    """
    if not sap_names:
        return {}
...
def _get_tipper_bd_hours(
    ...
    """SAP breakdown hours + count for ALL MAN tippers.
    Returns {machine_name: {"hours": float, "count": int}}
    """
    sql = text("""
        SELECT DESC_TECH_OBJECT,
...
def get_excavator_summary(db: Session, from_date: date, to_date: date) -> dict:
    ...
    for vdesc, sap_name, display_name, eng_hr in all_entries:
        ...
        mtbf      = round((period_hrs - bd_hr) / bd_count, 1) if bd_count > 0 else None
        machines.append({
            "vehicle_desc": vdesc,
    ...
    # Fleet-level MTTR / MTBF
    total_bd_count    = sum(m["bd_count"] for m in machines)
    total_bd_hrs      = sum(m["bd_hr"]    for m in machines)
    n_with_bd         = sum(1 for m in machines if m["bd_count"] > 0)
    fleet_mttr        = round(total_bd_hrs / total_bd_count, 1) if total_bd_count > 0 else None
    fleet_mtbf        = round(
        (n_with_bd * period_hrs - total_bd_hrs) / total_bd_count, 1
    ) if total_bd_count > 0 else None
